/* 2015 / 2016 /2017
   ...
   Designed for the Attiny85 and optimized for the #AttinyArcade platform. The source code is less than 8KB and the sketch
   ...
   rendering code and much of the game engine) in order to optimize for memory, improve responsiveness and allow new features on the limited hardware
   (added features include High score (saved to EEPROM), optional Ghost (or Shadow) Piece, Interrupt Handling and Hard-Mode functionality).
...
bool movePieceDown(void)
{
  ...
  if (checkCollision())
  {
    ...
    for (byte row = 0; row < VERTMAX; row++)
    { // scan the whole block (it's quick - there's no drawing to do)
      bool rowFull = 1;
      for (byte col = 0; col < HORIZ; col++)
      ...
      if (rowFull)
      {
        ...
        for (byte dropCol = 0; dropCol < HORIZ; dropCol++)
        { // for every column
          for (byte dropRow = row; dropRow < VERTMAX - 1; dropRow++)
...
void movePieceLeft(void)
{
  ...
  else if (response == 2)
  {
    ...
    for (byte i = 0; i < 4; i++)
    {
      if (currentPiece.blocks[3][i])
        wide = 1;
    }
    boolean narrow = true;
    for (byte i = 0; i < 4; i++)
    {
      if (currentPiece.blocks[2][i])
        narrow = false;
    }
    if (narrow)
      wide = -1;
  ...
  for (byte x = xpos; x < xpos + 6; x++)
  {
    ...
    for (byte lxn = 0; lxn < 8; lxn++)
    {
      if (blank)
        ...
        ssd1306_send_byte(pgm_read_byte(&font[4 + scoreOut[x - xpos]][7 - lxn]));
    }
    ssd1306_send_data_stop();
  }
}
...
void drawScreen(int startCol, int endCol, int startRow, int endRow, byte mode)
{
  ...
  for (byte col = startScreenCol; col < endScreenCol; col++)
  {
    ...
    if (mode == FULL)
      if (col < 6)
        for (byte numline = 0; numline < 8; numline++)
          ssd1306_send_byte(pgm_read_byte(&font[scoreOut[col] + 4][7 - numline]));
      else
