 ...
 *
 *  Any code not covered by the licenses below can be used freely with attribution. No warranties whatsoever are provided or implied.
 *
 *  ****************************************************************************************************
...
void shiftBits()
{
  ...
  else
  {
    ...
    if (averageDah > 1000)
      averageDah = 1000; // limiting slowest speed to about 3wpm !
...
void loop()
{
  ...
  while (digitalRead(0) == HIGH)
  {
    ...
    if (nowT - startT > 2000)
    {
      ...
      {
        mute = 1;
        ssd1306_char_f6x8(32, 7, "-- MUTE --");
      }
      else
      {
        mute = 0;
        ssd1306_char_f6x8(31, 7, "- SOUND ON -");
      }
      break;
    }
    if (sChange == 1)
