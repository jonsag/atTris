 ...
 *  BOTH BUTTONS TOGETHER move the frog forwards
 *
 *  HIGHLY RECOMMENDED:
 ...
 *  **Note that this highly size-optimized version requires modified library functions (which are in this source code file)
...
// The baseline speed - higher number is slower
...
byte frogRow;            // Row location of frog (there are 8, but 0 is the frog docks at the top and 7 is the start row)
...
{
  if (frogRow > 6 || frogRow < 1 || frogDead == 1)
  { // don't draw the frog when it's on the road or on logs - because they are moving, that's handled in the main drawing routine below- exception is when you are animating frog death
    if (frogRow == 1 || frogRow == 3)
    { // these allow for the blocks being shifted when animating the frog death on rows with logs
...
void drawGameScreen(byte mode)
{
  ...
  for (byte row = 0; row < 6; row += 2)
  {
    if (row >= 0 && row < 3)
      ...
      inverse = 0;              // draw everything (except the frog) in inverse video on the river rows (0,1,2)
    ssd1306_setpos(0, row + 1); // +1 because row 0 here is actually row 1 on the screen
    ssd1306_send_data_start();
    for (byte incr = 0; incr < 7 - blockShiftL; incr++)
      if (grid[row][15] == 0)
      {                       // cover the tiny bit to the far left of the screen up to wherever the main blocks will be drawn (depends on how far they are shifted)
    ...
    for (byte col = 0; col < 15; col++)
    {
      ...
      { // frog is amongst the cars and needs drawing
  ...
  if (frogColumn == 0)
    drawFrog(mode, 1); // this covers the exceptional case where the frog is in the far left colum, in which case the normal routine can't draw it when it's on the road
...
void playFrogger()
{
  ...
  while (lives >= 0)
  {
    ...
    if (watchDog >= 500)
      lives = -1; // Stop the game if nothing's happening - maybe triggered in someone's pocket so this is to save battery!
    ...
    if (moveForward == 1)
    {
      ...
      if (dockedFrogs >= 5)
      {
        ...
        initScreen();       // re initialize the position of game items
        resetDock(0);       // re initialize the dock
        dockedFrogs = 0;
    ...
    if (clickLock == 1 && millis() > clickBase + CLICKDELAY && digitalRead(2) == 0 && digitalRead(0) == 0 && analogRead(0) > 940)
    ...
    if (stopAnimate != 0)
    {
      ...
      frogColumn = 8;   // re initialize frog location
      frogRow = 7;
    }
...
void checkCollision(void)
{
  ...
  if ((frogRow < 7 && frogRow > 3) && (grid[frogRow - 1][frogColumn] != 0 || grid[frogRow - 1][frogColumn - 1] != 0))
...
void initScreen(void)
{
  ...
  if (level == 1)
  {
    gapCounter[5] = -14; // easiest setting, for start of game
  }
  if (level < 3)
  {
  ...
  if (level > 7)
  { // set smaller gaps between cars for levels over 7
    gapCounter[3] = -4;
    gapCounter[4] = -2;
    gapCounter[5] = -3;
  }
    ...
    for (byte col = 0; col < 15; col++)
    {
      if (counter[row] > 0)
      {
        if (14 - row > counter[row])
        {
          if (counter[row] == 1)
            if (stepMode == 1)
              stepMode = 2; // the next space is blank and we are drawing the middle - draw the end!
          if (row > 2)
            stepShift = 3;
          else
            stepShift = 0; // shift up to the trucks in the array
          if (row == 4)
            stepShift = 9; // shift up to the cars in the array - also theres no middle
          ...
          {
            grid[row][col] = 4 + stepMode + stepShift; // if you're on row zero (top row of logs) and you are above where crocs should be drawn, draw logs ...
          }
          else
            grid[row][col] = 10 + stepMode; // .. otherwise draw crocs
          if (stepMode == 0)
            stepMode = 1; // we've drawn the left side now switch to central sections
          if (stepMode == 2)
