 ...
 *  **Note that this highly size-optimized version requires modified library functions (which are in this source code file)
    ...
    As well as the above sources of inspiration, the pac-man screen artwork was inspired by elements of another clone found here https://sites.google.com/view/arduino-collection
...
// Debounce factor - around 50 works ok with AttinyArcade, but you might need to increase this number if you are using different hardware
...
void displayScreen(void)
{
  ...
  for (byte c = 0; c < 8; c++)
  {
    ...
    for (byte r = 0; r < 128; r++)
    {
      ...
      {
        if (c == 1)
          out |= (pgm_read_byte(&arrows[commandDir][r]));
        for (byte lxn = 3; lxn < 7; lxn++)
        {
          if (c == lxn && lives > 6 - lxn)
            out |= (pgm_read_byte(&PAClives[r]));
        }
      }
      ...
      for (byte lxn = 0; lxn < 3; lxn++)
      {
        ...
        { // draw the ghost if any of his body is on this column
      ...
      if (c > 0 && c < 7)
      {
        for (byte lxn = pgm_read_byte(&readRow[c - 1][0]); lxn < pgm_read_byte(&readRow[c - 1][1]); lxn++)
        {
          if (pillsActive[lxn])
          { // this pill is active
            if (r == pgm_read_byte(&pillLocations[lxn][0]))
            { // draw the pills if they appear here
...
void levelUp()
{
  ...
  if (maxGhosts < 3)
    maxGhosts++;
  if (releaseDelay > GHOST_DELAY_MINIMUM)
    releaseDelay -= GHOST_DELAY_REDUCTION;
...
{
  byte out = 0;
  if ((r >= locations[3][0]) && (r < locations[3][0] + 8))
  { // draw the pacMan if any of his body is on this column
...
int checkCollision(byte xpos, byte ypos)
{
  ...
  for (byte c = 0; c < 2; c++)
  { // for each of the two lines that the item might be on
    incr = (128 * (ypos / 8 + c)) + xpos;
    for (byte r = 0; r < 8; r++)
    { // for each of the eight bytes in the sprite
      if (c == 0)
      {
        if ((pgm_read_byte(&gameScreen[incr]) & (0xFF << ypos % 8)) != 0)
          returnValue = 1;
      }
      else
      {
        if ((pgm_read_byte(&gameScreen[incr]) & (0xFF >> (8 - ypos % 8))) != 0)
          returnValue = 1;
      }
      incr++;
    }
...
void moveGhosts(void)
{
  ...
  byte travelParam;     // zero for traveling in x, one for y
  ...
  for (byte lxn = 0; lxn < 3; lxn++)
  {
    if (ghostsActive[lxn] == 1)
    {
      ...
      if (gCount >= 5)
      {
        if (powerUp)
          travelDirection = 0;
        if (powerCounter > POWER_LENGTH - 50)
          flipFlop = !flipFlop;
        gCount = 0;
      }
      ...
      else
      {
        ...
        if (clash == 0 && random(100) > BRANCH_PROBABILITY)
          directions[lxn] = switchDirection;
...
void initLevel(void)
{
  ...
  for (byte lxn = 0; lxn < NO_PILLS; lxn++)
    pillsActive[lxn] = 1;
  bigPillsActive[0] = 1;
  bigPillsActive[1] = 1;
}
...
void playPacman()
{
  ...
  while (lives >= 0)
  {
    ...
    if (clickLock == 1 && millis() > clickBase + CLICKDELAY && digitalRead(2) == 0 && digitalRead(0) == 0)
