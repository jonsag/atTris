/* 2015 / 2016 /2017
   ...
   **Note that this highly size-optimized version requires modified library functions (which are in this source code file) and a modified font header which you'll find in the same place you found this .ino file
...
void playBird()
{
    ...
    for (i = 127 - gostep; i < 128; i++)
    {
      ...
      if (sinfactor > 0.97)
      {
        if (doneUpdate == 0)
        {
          ...
          while ((newheight > height - 7) && (newheight < height + 7))
            newheight = (random(7, 25));
          height = newheight;
   ...
   */
    ...
    if (totaldistance < 0)
      totaldistance = 0; // just in case - almost certainly would never happen!!
