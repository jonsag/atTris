...
void playUFO(void)
{
  ...
  while (stopAnimate == 0)
  {
    ...
    if ((fireCount == 0) && (playerOffset < 56))
    {
      playerOffset++;
    } // player gravit
...
void playStacker()
{
  ...
  while (stopAnimate == 0)
  {
    while (1)
    {
      ...
      if ((fireLock == 0) && (fire == 1))
      { // fire has been pressed - do stuff - check for alignment, update the array and decrement active row
      ...
      if (movecounter >= speedNow)
      {
        ...
        { // Moving right
          ...
          { // we've hit the end
            runCounter++;
            if (runCounter > 20)
            {                  // runCounter kills the game if no activity for 20 cycles
