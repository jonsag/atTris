 ...
 * drops enough to be noticeable when the button is pressed, but not enough to reset the chip!
 *
 * Also, from standby....
...
int fireXidx, fireYidx;     // mapping of player fire location onto array of aliens
int leftLimit;              // furthest left in x-axis pixels that the aliens are currently
...
void playSpaceAttack()
{
  ...
  while (stopAnimate == 0)
  {
    while (1)
    {
      ...
      // deal with inputs
      if (analogRead(0) < 940)
        fire = 1; // this reads the reset pin (pin 1) of the Attiny85 - to use it, see comments section above
      ...
      { // fire has been pressed and we're not currently firing - initiate fire!!
        playerFire[0] = 1;
        playerFire[1] = player + platformWidth / 2; // x position of new fire!
        playerFire[2] = 56;
      }
